  ...
  export function checkVerse(mode, element, arr) {
    if (mode === 'desktop') {
      return arr.forEach((item, i) => {
        return element[i].textContent === item ? element[i].style.backgroundColor = 'lawngreen' : element[i].style.backgroundColor = 'red';
      })
    }
    ...
    return arr.forEach((item, i) => {
      if (arr.indexOf(element[i].firstChild.textContent) + 1 === +element[i].children[0].textContent) {
